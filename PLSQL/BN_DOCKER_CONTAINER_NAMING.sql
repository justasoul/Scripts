...
with adjectives as (
select column_value adjective_value from table(varchar2_v100(
...
, nouns as (
select column_value noun_value from table(varchar2_v100(

     'albattani', -- // Muhammad ibn Jabir al-?arrani al-Battani was a founding father of astronomy. https://en.wikipedia.org/wiki/Mu%E1%B8%A5ammad_ibn_J%C4%81bir_al-%E1%B8%A4arr%C4%81n%C4%AB_al-Batt%C4%81n%C4%AB
    ...
    'bhabha', -- // Homi J Bhabha - was an Indian nuclear physicist, founding director, and professor of physics at the Tata Institute of Fundamental Research. Colloquially known as 
    ...
    'Bill', -- Gates III is an American business magnate, philanthropist, investor, computer programmer, and inventor. https://en.wikipedia.org/wiki/Bill_Gates
    'gates', -- // William Henry 
    ...
    'debugging', --  for fixing computer glitches. https://en.wikipedia.org/wiki/Grace_Hopper
    'hopper', -- // Grace Hopper developed the first compiler for a computer programming language and  is credited with popularizing the term 
    ...
    'knuth', -- // Donald Knuth - American computer scientist, author of 
...
Select rownum row_num, count(1) over () cnt, A.*, N.* 
  from adjectives A, nouns N
)

Select AP.adjective_value || '_' || AP.noun_value nome 
  from all_possibilities AP       
 where AP.row_num = trunc(dbms_random.value(low=> 1, high=> AP.cnt))  
   and rownum = 1   
